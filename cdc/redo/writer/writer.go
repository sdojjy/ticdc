...
func NewLogWriter(ctx context.Context, cfg *LogWriterConfig) (*LogWriter, error) {
	...
	logWriter.metricTotalRowsCount = redoTotalRowsCountGauge.WithLabelValues(cfg.ChangeFeedID.ID)
...
func (l *LogWriter) runGC(ctx context.Context) {
	...
	for {
		...
		case <-ctx.Done():
			err := l.Close()
			if err != nil {
				log.Error("runGC close fail", zap.String("changefeed", l.cfg.ChangeFeedID.ID), zap.Error(err))
			}
		case <-ticker.C:
			err := l.gc()
			if err != nil {
				log.Error("redo log GC fail", zap.String("changefeed", l.cfg.ChangeFeedID.ID), zap.Error(err))
			}
		}
	}
}
...
func (l *LogWriter) Close() error {
	redoTotalRowsCountGauge.DeleteLabelValues(l.cfg.ChangeFeedID.ID)
