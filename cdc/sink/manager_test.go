		...
		go func() {
			...
			for j := 1; j < rowNum; j++ {
				if rand.Intn(10) == 0 {
					...
					_, err := tableSink.FlushRowChangedEvents(ctx, 1, resolvedTs)
			...
			_, err := tableSink.FlushRowChangedEvents(ctx, 1, uint64(rowNum))
			c.Assert(err, check.IsNil)
		}()
	}
	wg.Wait()
...
func (s *managerSuite) TestManagerAddRemoveTable(c *check.C) {
	...
	runTableSink := func(ctx context.Context, index int64, sink Sink, startTs uint64) {
		...
		for {
			...
			_, err := sink.FlushRowChangedEvents(ctx, 1, resolvedTs)
			if err != nil {
...
func BenchmarkManagerFlushing(b *testing.B) {
	...
	for i := 1; i < goroutineNum; i++ {
		...
		go func() {
			for j := 1; j < rowNum; j++ {
				if j%2 == 0 {
					_, err := tableSink.FlushRowChangedEvents(context.Background(), int64(i), uint64(j))
					if err != nil {
						b.Error(err)
					}
				}
	...
	for i := 0; i < b.N; i++ {
		_, err := tableSink.FlushRowChangedEvents(context.Background(), 1, uint64(rowNum))
		if err != nil {
			b.Error(err)
		}
	}
