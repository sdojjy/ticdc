...
func (k *kafkaSaramaProducer) AsyncSendMessage(
	...
	failpoint.Inject("KafkaSinkAsyncSendError", func() {
		...
		log.Info("failpoint error injected", zap.String("changefeed", k.id.ID), zap.Any("role", k.role))
...
func (k *kafkaSaramaProducer) stop() {
	...
	log.Info("kafka producer closing...", zap.String("changefeed", k.id.ID), zap.Any("role", k.role))
	close(k.closeCh)
}

// Close closes the sync and async clients.
func (k *kafkaSaramaProducer) Close() error {
	log.Info("stop the kafka producer", zap.String("changefeed", k.id.ID), zap.Any("role", k.role))
	k.stop()
...
func kafkaClientID(role, captureAddr string, changefeedID model.ChangeFeedID, configuredClientID string) (clientID string, err error) {
	if configuredClientID != "" {
		...
		clientID = fmt.Sprintf("TiCDC_sarama_producer_%s_%s_%s", role, captureAddr, changefeedID.String())
