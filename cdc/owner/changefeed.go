...
func (c *changefeed) Tick(ctx cdcContext.Context, state *orchestrator.ChangefeedReactorState, captures map[model.CaptureID]*model.CaptureInfo) {
	...
	if c.initialized {
		...
		if costTime > changefeedLogsWarnDuration {
			log.Warn("changefeed tick took too long", zap.String("changefeed", c.id.String()), zap.Duration("duration", costTime))
		}
		c.metricsChangefeedTickDuration.Observe(costTime.Seconds())
	}

	if err != nil {
		log.Error("an error occurred in Owner", zap.String("changefeed", c.state.ID.String()), zap.Error(err))
		var code string
...
func (c *changefeed) tick(ctx cdcContext.Context, state *orchestrator.ChangefeedReactorState, captures map[model.CaptureID]*model.CaptureInfo) error {
	...
	if costTime > schedulerLogsWarnDuration {
		log.Warn("scheduler tick took too long", zap.String("changefeed", c.id.String()), zap.Duration("duration", costTime))
	}
	if err != nil {
		return errors.Trace(err)
	}
...
LOOP:
	...
	checkpointTs := c.state.Info.GetCheckpointTs(c.state.Status)
	log.Info("initialize changefeed", zap.String("changefeed", c.state.ID.String()),
...
func (c *changefeed) releaseResources(ctx cdcContext.Context) {
	...
	log.Info("close changefeed", zap.String("changefeed", c.state.ID.String()),
	...
	if err := c.sink.close(canceledCtx); err != nil {
		log.Warn("Closing sink failed in Owner", zap.String("changefeed", c.state.ID.String()), zap.Error(err))
	}
	c.wg.Wait()
...
func (c *changefeed) redoManagerCleanup(ctx context.Context) {
	if c.isRemoved {
		...
		if c.redoManager == nil {
			...
			if err != nil {
				log.Error("create redo manager failed", zap.String("changefeed", c.id.String()), zap.Error(err))
				return
			}
			c.redoManager = redoManager
		}
		...
		if err != nil {
			log.Error("cleanup redo logs failed", zap.String("changefeed", c.id.String()), zap.Error(err))
		}
	}
}
...
func (c *changefeed) asyncExecDDL(ctx cdcContext.Context, job *timodel.Job) (done bool, err error) {
	if job.BinlogInfo == nil {
		log.Warn("ignore the invalid DDL job", zap.String("changefeed", c.id.String()),
			zap.Reflect("job", job))
		return true, nil
	}
	...
	if cyclicConfig.IsEnabled() && !cyclicConfig.SyncDDL {
		log.Info("ignore the DDL job because cyclic config is enabled and syncDDL is false",
			zap.String("changefeed", c.id.String()), zap.Reflect("job", job))
		return true, nil
	}
	if c.ddlEventCache == nil || c.ddlEventCache.CommitTs != job.BinlogInfo.FinishedTS {
		ddlEvent, err := c.schema.BuildDDLEvent(job)
		if err != nil {
			log.Error("build DDL event fail", zap.String("changefeed", c.id.String()),
		...
		if err != nil {
			log.Error("add special comment fail", zap.String("changefeed", c.id.String()),
...
func (c *changefeed) Close(ctx cdcContext.Context) {
	...
	costTime := time.Since(startTime)
	if costTime > changefeedLogsWarnDuration {
		log.Warn("changefeed close took too long", zap.String("changefeed", c.id.String()), zap.Duration("duration", costTime))
	}
