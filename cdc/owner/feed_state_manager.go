...
func (m *feedStateManager) PushAdminJob(job *model.AdminJob) {
	...
	default:
		log.Panic("Can not handle this job", zap.String("changefeed", m.state.ID.String()),
...
func (m *feedStateManager) handleAdminJob() (jobsPending bool) {
	...
	log.Info("handle admin job", zap.String("changefeed", m.state.ID.String()), zap.Reflect("job", job))
	switch job.Type {
	case model.AdminStop:
		switch m.state.Info.State {
		case model.StateNormal, model.StateError:
		default:
			log.Warn("can not pause the changefeed in the current state", zap.String("changefeed", m.state.ID.String()),
				zap.String("changefeedState", string(m.state.Info.State)), zap.Any("job", job))
			return
		}
		m.shouldBeRunning = false
	...
	case model.AdminRemove:
		...
		default:
			log.Warn("can not remove the changefeed in the current state", zap.String("changefeed", m.state.ID.String()),
				zap.String("changefeedState", string(m.state.Info.State)), zap.Any("job", job))
			return
		}
		m.shouldBeRunning = false
		...
		log.Info("the changefeed is removed", zap.String("changefeed", m.state.ID.String()), zap.Uint64("checkpointTs", checkpointTs))
	case model.AdminResume:
		switch m.state.Info.State {
		case model.StateFailed, model.StateError, model.StateStopped, model.StateFinished:
		default:
			log.Warn("can not resume the changefeed in the current state", zap.String("changefeed", m.state.ID.String()),
				zap.String("changefeedState", string(m.state.Info.State)), zap.Any("job", job))
			return
		}
		m.shouldBeRunning = true
	...
	case model.AdminFinish:
		...
		default:
			log.Warn("can not finish the changefeed in the current state", zap.String("changefeed", m.state.ID.String()),
				zap.String("changefeedState", string(m.state.Info.State)), zap.Any("job", job))
			return
		}
		m.shouldBeRunning = false
	...
	default:
		log.Warn("Unknown admin job", zap.Any("adminJob", job), zap.String("changefeed", m.state.ID.String()))
	}
	return
}
...
func (m *feedStateManager) errorsReportedByProcessors() []*model.RunningError {
	...
	for captureID, position := range m.state.TaskPositions {
		if position.Error != nil {
			...
			log.Error("processor report an error", zap.String("changefeed", m.state.ID.String()), zap.String("captureID", captureID), zap.Any("error", position.Error))
			m.state.PatchTaskPosition(captureID, func(position *model.TaskPosition) (*model.TaskPosition, bool, error) {
				if position == nil {
					return nil, false, nil
				}
...
func (m *feedStateManager) handleError(errs ...*model.RunningError) {
	...
	if time.Since(m.lastErrorTime) < m.backoffInterval {
		...
		if m.backoffInterval == backoff.Stop {
			...
			log.Info("changefeed restart backoff interval is changed", zap.String("changefeed", m.state.ID.String()),
